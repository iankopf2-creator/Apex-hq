...
const NICHE_FIX: Record<string, string> = {
  ...
  smart_panel:
    "Add a quote-first smart panel page (main panel amperage + main breaker size; full panel replacement vs smart overlay / retrofit when true; circuit count / breaker map / load inventory when true; EV / solar / battery / generator / transfer switch / surge SPD coexistence when true; utility interconnection / demand response / TOU when true; Wi-Fi / app / monitoring honesty when true — no fake always-online; permit + licensed electrician honesty when true; assess before firm price) so callers get a real quote — not bait flat $/circuit or $/panel or fake same-day install; distinct from panel_upgrade, transfer_switch, battery_storage, generator, whole_home_surge, ev_charger, solar, and electrician call-first emergency.",
  countertops:
