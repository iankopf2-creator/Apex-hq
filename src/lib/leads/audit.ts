...
const NICHE_FIX: Record<string, string> = {
  ...
  dryer_vent_cleaning:
    "Add a quote-first dryer vent cleaning page (stories/floors + vent length + exterior termination access + roof vs wall + bird nest/lint load + gas vs electric dryer + crawl/attic access + weather assess before firm price; fire-risk educational only — no scare fake emergency; licensed honesty when true) so callers get a real quote — not bait flat $/vent or fake same-day clear; distinct from HVAC duct cleaning and chimney sweeping.",
  countertops:
