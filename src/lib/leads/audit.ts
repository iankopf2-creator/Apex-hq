...
const NICHE_FIX: Record<string, string> = {
  ...
  wildlife_removal:
    "Add a quote-first wildlife removal page (species ID + entry points + attic/crawl access + exclusion vs live-trap vs one-way door + cleanup/sanitation + seasonal nesting + local permit/wildlife rehab assess before firm price; licensed/insured / state wildlife permit honesty only when true; educational only on local laws — not legal advice) so callers get a real quote — not bait flat $/animal or $/attic or fake same-day guaranteed catch; distinct from pest_control insects call-first, tree_service storm, junk_removal, mold_remediation, and handyman.",
  countertops:
