...
const NICHE_FIX: Record<string, string> = {
  ...
  kitchen_hood_cleaning:
    "Add a quote-first kitchen hood cleaning page (commercial restaurant exhaust hood + hood type canopy/island/pizza oven + grease load + stories/roof access + fan & duct path + after-hours schedule assess before firm price; NFPA-96 / fire-code educational only — no scare fake emergency; licensed/certified honesty when true) so callers get a real quote — not bait flat $/hood or fake same-day clear; distinct from HVAC duct cleaning, dryer vent cleaning, and janitorial.",
  countertops:
