...
const NICHE_FIX: Record<string, string> = {
  ...
  battery_storage:
    "Add a quote-first battery storage page (residential home battery / ESS install — home size / electrical panel capacity + main breaker / existing solar or generator when true / backup loads critical vs whole-home / utility interconnection + net-metering educational only (no guarantee) / incentive/tax-credit educational only (no guarantee) / outdoor vs garage pad space / HOA/permit / licensed electrician-ESS contractor honesty when true; assess before firm price) so callers get a real quote — not bait flat $/kWh or $/kW or fake same-day install; distinct from solar, generator, electrician, ev_charger, geothermal, heat_pump, mini_split, and hvac.",
  countertops:
