...
- [x] Theme niche panel_upgrade (quote_first) — deep load-center steel `#1e242b` + copper bus-bar accent `#c17a3a`; existing panel amperage (100A vs 200A vs 400A when true) / main breaker size + bus rating / available slots / tandem vs full-size / service entrance / meter / utility disconnect / NEC load calculation (EV charger / battery storage / solar / generator / HVAC when true) / indoor vs outdoor / weather location / aluminum vs copper service when true / permit + licensed electrician honesty when true; assess before firm price; no bait flat $/amp or $/slot; no fake same-day upgrade; no scare fire-risk copy; no inventing NEC as Apex legal advice; no competitor brand cloning (Square D / Siemens / Eaton / Cutler-Hammer as Apex product); distinct from electrician, battery_storage, ev_charger, solar, generator, hvac
