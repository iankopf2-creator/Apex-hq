...
- [x] Theme niche radon_mitigation (quote_first) — radon charcoal/slate + test-kit amber (#27272a / #d97706); test-first (short/long/continuous) before mitigation design; entry points/soil gas/foundation type (slab/crawl/basement)/suction pit vs crawl membrane fan/discharge height & neighbor setbacks/electrical for fan/post-mitigation retest; NRPP/NRSB or state cert honesty only when true; permit/HOA when true; educational only on health risk (not medical); no bait flat $/system; no fake guaranteed zero radon or same-day cure; distinct from crawl_space_encapsulation, basement_waterproofing, french_drain, foundation_repair, water_damage, mold_remediation, insulation, HVAC duct work
